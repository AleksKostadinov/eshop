...
class CustomLoginView(LoginView):
    ...
    def form_invalid(self, form):
        if (
            ...
            and "already in use" in form.errors["email"][0].lower()
        ):
            messages.error(
                self.request, "This email is already associated with another account."
            )
        else:
...
class EditProfileView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        if account_form.is_valid() and profile_form.is_valid():
            try:
                account_form.cleaned_data["phone_number"] = clean_phone_number(
                    account_form.cleaned_data["phone_number"]
                )
                account_form.cleaned_data["first_name"] = clean_first_name(
                    account_form.cleaned_data["first_name"]
                )
                account_form.cleaned_data["last_name"] = clean_last_name(
    ...
    def form_valid(self, form):
        ...
        if success:
            if current_password == new_password:
                messages.error(
                    self.request, "New password cannot be same as current password."
                )
...
class OrderDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        order_detail = OrderProduct.objects.filter(
            order__order_number=self.object.order_number
        )
...
class SubscribeView(View):
    def post(self, request, *args, **kwargs):
        ...
        messages.success(
            request, f"The email {email} was successfully subscribed to our newsletter!"
        )
        return redirect(request.META.get("HTTP_REFERER", "/"))
...
class UnsubscribeView(View):
    def post(self, request, *args, **kwargs):
        ...
        if not email:
            messages.error(
                request,
                "You must provide a valid email to unsubscribe from the Newsletter",
            )
            return redirect("/")
        ...
        messages.success(
            request,
            f"The email {email} was successfully unsubscribed from our newsletter.",
        )
        return redirect(request.META.get("HTTP_REFERER", "/"))


class WishlistAddRemoveView(LoginRequiredMixin, View):
...
class WishlistView(ListView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        users_wishlist = Product.objects.filter(users_wishlist=self.request.user)
        context['users_wishlist'] = users_wishlist
        return context
